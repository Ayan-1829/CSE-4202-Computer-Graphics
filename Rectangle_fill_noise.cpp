...
void BFA_(int x, int y, int b_col, int f_col)
{
    // window check
    if (x >= window_x || x <= 0 || y >= window_y || y <= 0)
    {
    ...
    putpixel(x, y, f_col);
    // delay(5);
    BFA_(x + 1, y, b_col, f_col);
    BFA_(x, y + 1, b_col, f_col);
    BFA_(x - 1, y, b_col, f_col);
    BFA_(x, y - 1, b_col, f_col);
}
...
int main()
{
    ...
    BFA(seed_x, seed_y, boundary_color, 7);

    add_noise(seed_x, y2, background_color);
    
    BFA_(seed_x, seed_y, 7, fill_color);
